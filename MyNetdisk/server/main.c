...
int main(int argc, char* argv[])
{
    ...
    // 总体结构：父进程(单线程), 子进程(主线程+N子线程)(主线程负责短命令,子线程负责长命令)
    pipe(exitPipe);
    if(fork()) {
        close(exitPipe[0]);
    ...
    while(1) {
        int readyNum = epoll_wait(epfd, evs, SIZE(evs), 1000);
        for(int i = 0; i < readyNum; i++) {
            if(evs[i].data.fd == sfd) { // 主线程收到来自客户端的请求
                ...
                // if(长命令)
                    ...
                    // 验证成功，netfd放入短连接队列
                    pthread_mutex_lock(&threadPool.lkQueue.mutex);
                    enQueue(&threadPool.lkQueue, netfd); // 入队
                    pthread_cond_broadcast(&threadPool.lkQueue.cond); // 通知子进程
                    pthread_mutex_unlock(&threadPool.lkQueue.mutex);
                }
            ...
            if(evs[i].data.fd == exitPipe[0]) { // 主线程收到关闭信号
                ...
                pthread_cond_broadcast(&threadPool.lkQueue.cond); // 异步唤起同步
                pthread_mutex_unlock(&threadPool.lkQueue.mutex);
