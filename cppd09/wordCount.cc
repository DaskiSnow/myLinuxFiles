...
private:
    ...
    static bool cmp(Record & a, Record & b);
    friend void test();
};
...
void Dictionary::store(const string &filename) {
    ...
    for(auto & rec : _dict) {
        char linebuf[128] = {0};
        sprintf(linebuf, "%-15s\t%4d", rec._word.c_str(), rec._frequency);
        ofs << linebuf << endl;
    }
...
// 升序
bool Dictionary::cmp(Record & a, Record & b) {
