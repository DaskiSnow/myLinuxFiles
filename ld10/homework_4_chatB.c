...
{
    // ./homework_2_chatB 1.pipe 2.pipe
    ARGS_CHECK(argc, 3);
    ...
    while(1) {
        ...
        if(FD_ISSET(fdr, &rdset)){
            ...
            if(rret == 0) {
                printf("--------Detect [off signal] from ChatA. Chat is closing!\n");
                break;
            }
            ...
            printf("====(%d/%d/%d weekday-%d %d:%d:%d)From ChatA: %s",
        ...
        if(time_cur - time_la > 50) {
            printf("--------No message received from ChatA for 10s. Chat is closing!\n");
