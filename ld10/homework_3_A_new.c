...
int main(int argc, char* argv[])
{
    ...
    meta_info.size = meta_stat.st_size;

    // 头部
    head_t head;
    ssize_t wret;
    // 发送头部后，再发送元信息
    head.length = sizeof(meta_info);
    write(fdw, &head, sizeof(head));
    ERROR_CHECK(wret, -1, "write head");
    wret = write(fdw, &meta_info, sizeof(meta_info));
    ...
    while(1) {
        ...
        if (n == 0) break;
        head.length = n;
        wret = write(fdw, &head, sizeof(head));
        ERROR_CHECK(wret, -1, "write head");
